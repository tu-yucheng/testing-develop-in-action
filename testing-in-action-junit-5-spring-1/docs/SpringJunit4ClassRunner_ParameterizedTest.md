...
从测试中可以看出，我们的Controller接收一个name作为路径参数，并相应地返回用户角色。
...
幸运的是，Spring提供了多种方法来实现这一点。我们将在以下部分介绍这些内容。
...
除了SpringClassRule，我们还必须使用SpringMethodRule。它为TestContextManager提供了实例级和方法级的功能。
